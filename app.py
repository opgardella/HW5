...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://postgres:si364@localhost/gardellaHW5"
## Provided:
...
class UpdatePriorityForm(FlaskForm):
    updated_priority = StringField("What is the new priority of this item?", validators=[Required()])
    update = SubmitField("Update")
...
def update(item):
    ...
    if form.validate_on_submit():
        ...
        items.priority = form.updated_priority.data
        db.session.commit()
...
def delete(lst):
    ...
    todolist = TodoList.query.filter_by(title = lst).first()
